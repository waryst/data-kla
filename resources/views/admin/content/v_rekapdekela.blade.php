        ...
        <div class="container-fluid">
            <fieldset class="scheduler-border">
                ...
                <div class="col-12 my-2">
                    <div class="card">
                        <h2 class="card-title mx-auto font-weight-bold mt-">PROGRES PENGUMPULAN DATA DEKELA</h2>
                            ...
                            <table id="example1" class="table table-head-fixed table-bordered table-hover text-wrap">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center align-middle">Progres Pengumpulan Data Dekela desa
                                            perkecamatan
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($kecamatan as $item)
                                        <?php
                                        $progress = round(($item->desa->count() / $item->desa_count) * 100);
                                        if ($progress < 50) {
                                            $warna = 'bg-danger';
                                        } elseif ($progress < 70) {
                                            $warna = 'bg-warning';
                                        ...
                                        <tr>
                                            ...
                                            <td><a href="{{ url('rekap_dekela/' . $item->id) }}">Kecamatan
                                                    {{ $item->title }}</a></td>
                                            <td>
                                                <div class="progress progress-md">
                                                    <div class="progress-bar {{ $warna }}"
                                                        ...
                                                        <span
                                                            class="font-weight-bold">{{ $item->desa->count() }}/{{ $item->desa_count }}
                                                            ({{ $progress }}%)
                                                        </span>
                                                    </div>
